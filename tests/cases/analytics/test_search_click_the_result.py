...
@allure.feature("点击搜索结果埋点")
@allure.story("点击搜索结果埋点测试")
@pytest.mark.analytics
def test_STB_FU_PO_2510230123(analytics_test):
    # 目标埋点事件ID
    search_click_the_result = "110070002"
    
    with allure.step(f"点击搜索结果 [{search_click_the_result}]"):
        # 设置测试环境
        analytics_test.setup_for_event_test(event_id=search_click_the_result) 
        ...
        try:
            ...
            success, events = analytics_test.execute_path_operation_with_analytics(
                event_id=search_click_the_result,
                path_operations=path_operations,
                target_action=last_action,
                path_description="点击搜索结果",
                target_description="点击搜索结果"
            )
            ...
            assert success, f"埋点验证失败：未找到事件ID为 {search_click_the_result} 的触发记录，或时间不匹配"
            ...
            try:
                ...
                analytics_test.teardown_event_test(search_click_the_result)
