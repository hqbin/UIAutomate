...
@allure.feature("应用安装埋点")
@allure.story("应用安装埋点测试")
@pytest.mark.analytics
def test_STB_FU_PO_2510230059(analytics_test):
    # 目标埋点事件ID
    app_install = "110060003"
    
    with allure.step(f"应用安装 [{app_install}]"):
        # 设置测试环境
        analytics_test.setup_for_event_test(event_id=app_install)
        ...
        try:
            ...
            success, events = analytics_test.execute_path_operation_with_analytics(
                event_id=app_install,
                path_operations=path_operations,
                target_action=last_action,
                path_description="应用安装",
                target_description="应用安装"
            )
            ...
            assert success, f"埋点验证失败：未找到事件ID为 {app_install} 的触发记录，或时间不匹配"
            ...
            try:
                ...
                analytics_test.teardown_event_test(app_install)