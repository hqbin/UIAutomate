...
def test_STB_FU_PO_2510230061(analytics_test):
    # 目标埋点事件ID
    apps_search = "110070002"
    
    with allure.step(f"apps_search搜索 [{apps_search}]"):
        # 设置测试环境
        analytics_test.setup_for_event_test(event_id=apps_search)
        ...
        try:
            ...
            success, events = analytics_test.execute_path_operation_with_analytics(
                event_id=apps_search,
                path_operations=path_operations,
                target_action=last_action,
                path_description="apps_search搜索",
                target_description="apps_search搜索"
            )
            ...
            assert success, f"埋点验证失败：未找到事件ID为 {apps_search} 的触发记录，或时间不匹配"
            ...
            try:
                ...
                analytics_test.teardown_event_test(apps_search)
